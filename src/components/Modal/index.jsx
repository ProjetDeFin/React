...
export default function Modal({ options, enableContractInput }) {
    ...
    const getAvailableOptions = (currentId) => {
        ...
        return selectOptions.filter(option => !selectedValuesArray.includes(option.value) || selectedValues[currentId] === option.value);
    };
        ...
        <div className="modal">
            ...
            {selectList.map((selectId) => (
                <div key={selectId} className="select-container">
                    <select onChange={(e) => handleSelectChange(selectId, e.target.value)} value={selectedValues[selectId] || ''}>
                    ...
                    <button className="delete" onClick={() => handleSelectRemove(selectId)}>
